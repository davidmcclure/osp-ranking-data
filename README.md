...
We're working on expanding the corpus in a couple of ways. We're planning to scrape more documents from the web, and we're also hoping to partner with universities to get access to archived collections of syllabi that aren't posted online. For now, though, it's important to keep in mind that this data is pulled from one particular subset of syllabi, and can't yet be considered a comprehensive authority on text assignment trends. The results are suggestive of larger trends (things like - the great books are doing just fine), but it's probably not a good idea at that stage to read too much into really fine-grained distinctions in the data.
